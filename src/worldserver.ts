...
import { Spawn, List, Population, Blink, Destroy, Move, Kill, Damage } from "./message";
...
export class 世界服务器{
    ...
    pushSpawnsToPlayer (player, ids) {
        ...
        _.each(ids, function(id) {
            ...
            if(entity) {
                self.pushToPlayer(player, new Spawn(entity));
            }
        });
            ...
            this.map.forEachGroup(function(id) {
                ...
                if(self.groups[id].incoming.length > 0) {
                    spawns = _.each(self.groups[id].incoming, function(entity) {
                        if(entity instanceof Player) {
                            self.pushToGroup(id, new Spawn(entity), entity.id);
                        } else {
                            self.pushToGroup(id, new Spawn(entity));
                        }
                    });
        ...
        if(item) {
            item.handleDespawn({
                ...
                blinkCallback:function() {
                    self.pushToAdjacentGroups(item.group, new Blink(item));
                } ,
                blinkingDuration: 4000,
                despawnCallback:function() {
                    self.pushToAdjacentGroups(item.group, new Destroy(item));
    ...
    updatePopulation (totalPlayers?) {
        ...
        this.pushBroadcast(new Population(this.playerCount, totalPlayers));
    }
}